        ...
        if demandPosit > truDemand:
            ...
            #otherwise, the player is given a cost for their request to be granted, reduced by CHA
                ...
                while(notDone):
                    ...
                    if event.event_type == keyboard.KEY_DOWN and event.name == 'z':
                        if (target.getconvType() == "Carnivore" and (truDemand * mc.getLevel()) > mc.bCount) or (target.getconvType() == "Herbivore" and (truDemand * mc.getLevel()) > mc.sCount):
                            ...
                            if target.getconvType() == "Carnivore":
                                mc.bCount -= (truDemand * mc.getLevel())
                            else:
                                mc.sCount - (truDemand * mc.getLevel())
...
def askQuestion(mc, convoData, demandPosit):
    ...
    while(running):
        ...
        if choice == -2:
            ...
            if (iptedLst[cOption][1] == "*" and mc.sCount < max((iptedLst[cOption][2]*mc.getLevel() - mc.getCHA()), 1)) or (iptedLst[cOption][1] == "$" and mc.bCount < max((iptedLst[cOption][2]*mc.getLevel() - mc.getCHA()), 1)):
                ...
                #player spends sticks and stones if they selected a * option
                    ...
                    mc.sCount -= max((iptedLst[1][2]*mc.getLevel() - mc.getCHA()), 1)
                    print("You used some sticks and stones")
                #player spends blood and bones if they selected a $ option
                elif iptedLst[cOption][1] == "$":
                    mc.bCount -= max((iptedLst[1][2]*mc.getLevel() - mc.getCHA()), 1)
