...
##The equipment class values are kept public, as they are broadly used as a custom type for pmember
class equipment():
...
#item is also used a custom type more than as an object, so variables are kept public.
class item():
...
#the equipment slots are filled with equipment objects
    ...
    def setSpells(self):
        con = sqlite3.connect("strandedData.db")
        cur = con.cursor()
        spellData = cur.execute("SELECT spellData.spellID FROM entSpells INNER JOIN spellData ON spellData.spellID = entSpells.spellID WHERE entSpells.entID ="+str(self._entID)+" AND entSpells.spellRank="+str(self.level)).fetchall()
    ...
    def printStats(self):
        print(self._name, "  LVL:", self.level)
    ...
    def makeCombattant(self):
        return ally(self._sprID,self._name, self.HP, self.cHP, self.MP, self.cMP, self.ATK, self.DEF, self.mATK, self.HIT, self.DODGE, self.CRIT,  self.level, self.CHA, self.spellList, self.bCount, self.sCount)
...
def updatedParty(partyLst, updates):
    ...
    if updates[3] == "Lost":
        partyLst[0].bCount = int(partyLst[0].bCount /2)
        partyLst[0].sCount = int(partyLst[0].sCount /2)
